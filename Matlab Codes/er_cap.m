...
C = C/N_iter;
figure, plot(SNR_dB,C(1,:),'g-o', SNR_dB,C(2,:),'k-<', SNR_dB,C(3,:),'b-s','LineWidth',1.2);
hold on, plot(SNR_dB,C(4,:),'m->', SNR_dB,C(5,:),'r-^','LineWidth',1.2);
xlabel('SNR[dB]'); ylabel('bps/Hz');
s1='{\it M_T}=1,{\it M_R}=1'; s2='{\it M_T}=1,{\it M_R}=2'; 
s3='{\it M_T}=2,{\it M_R}=1'; s4='{\it M_T}=2,{\it M_R}=2'; s5='{\it M_T}=4,{\it M_R}=4';
